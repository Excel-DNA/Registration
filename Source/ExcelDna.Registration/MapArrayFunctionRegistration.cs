...
namespace ExcelDna.Registration
{
    ...
    public static class MapArrayFunctionRegistration
    {
        ...
        private class ShimParameter
        {
            ...
            /// <summary>
            /// Can be null. A function built from config to convert each enumerated item.
            /// </summary>
            private Func<object, object> ItemConverter { set; get; }
            ...
            static LambdaExpression CastResult(LambdaExpression lambda, Type type)
            {
            ...
            static LambdaExpression CastParameter(LambdaExpression lambda, Type type)
            {
            ...
            static LambdaExpression CastParamAndResult(LambdaExpression lambda, Type t)
            {
                return CastResult(CastParameter(lambda, t), t);
            }

            static Func<object, object> GetItemConverter(LambdaExpression lambda)
            {
                ...
                : this(type, reg.CustomAttributes)
            {
                ...
                if (config != null && EnumeratedType != null)
                {
                    var itemConverterLambda =
                        ParameterConversionRegistration.GetParameterConversion(config, EnumeratedType, reg) ??
                        TypeConversion.GetConversion(typeof(object), EnumeratedType);
                    ItemConverter = GetItemConverter(itemConverterLambda);
                }
            }

            ExcelReturnRegistration ReturnRegistration { set; get; }

            private delegate void SetCurrentCellDelegate();

            private SetCurrentCellDelegate SetCurrentCell;
                ...
                : this(type, reg.CustomAttributes)
            {
                ...
                {
                    var itemConverterLambda =
                        ParameterConversionRegistration.GetReturnConversion(config, EnumeratedType, reg,
                            setCurrentCellInMarshalByRefCache: false) ??
                        TypeConversion.GetConversion(typeof(object), EnumeratedType);
                    ItemConverter = GetItemConverter(itemConverterLambda);
                    bool marshalByRef = config.IsMarshalByRef(EnumeratedType);
                    if (marshalByRef)
                        SetCurrentCell = config.ReferenceMarshaller.SetCurrentCell;
                }
            }
            ...
            public object ConvertShimToTarget(object inputObject)
            {
                                ...
                                inputPropertyCols.Select(
                                    prop =>
                                        ConvertFromExcelObject(objectArray2D[row + 1, prop.Item2],
                                            prop.Item1.PropertyType, ItemConverter)).ToArray());
                        }
                        catch (MissingMethodException)
                        {
                            ...
                            foreach (var prop in inputPropertyCols)
                                prop.Item1.SetValue(inputRecord,
                                    ConvertFromExcelObject(objectArray2D[row + 1, prop.Item2],
                                        prop.Item1.PropertyType, ItemConverter), null);
                        }
                ...
                if (EnumeratedType != null)
                {
                    ...
                    if (objectArray == null)
                    {
                        ...
                        result.SetValue(ConvertFromExcelObject(inputObject, EnumeratedType, ItemConverter), 0);
                    }
                    else
                    {
                        ...
                        for (int i = 0; i != nItems; ++i)
                            result.SetValue(ConvertFromExcelObject(objectArray[nRows==1? 0: i, nRows == 1? i: 0], EnumeratedType, ItemConverter), i);
                    }
                    return result;
                }

                // it's a simple value type
                return ConvertFromExcelObject(inputObject, this.Type, ItemConverter);
            }
            ...
            public object ConvertTargetToShim(object outputObject)
            {
                ...
                if (EnumeratedType != null)
                {
                    ...
                    var returnRecordArray = toArray2D.Invoke(null, new object[] { outputObject, Orientation.Vertical, ItemConverter }) as object[,];
                    if (returnRecordArray == null)
            ...
            private static object ConvertFromExcelObject(object from, Type toType, Func<object, object> itemConverter)
            {
                object result = null;
                ...
                {
                    result = DateTime.FromOADate((double)@from);
                }
                else if (toType == typeof(DateTime) && (@from is int))
                {
                    result = DateTime.FromOADate((int)@from);
                }
                else if (@from is ExcelEmpty)
                {
                    ...
                    if (toType.IsValueType)
                        ...
                        result = String.Empty;
                    else if (toType.GetConstructor(Type.EmptyTypes) != null)
                        result = Activator.CreateInstance(toType);
                }
                else if (itemConverter != null)
                    result = itemConverter(@from);
                else
        ...
        public static object[,] ToArray2D<T>(this IEnumerable<T> input, Orientation orient, Func<T,object> itemConverter = null)
        {
